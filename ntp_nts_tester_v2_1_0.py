...
class NTPApp:
    def __init__(self, root):
        ...
        self.peer_suite_running = False
        self.peer_suite_total = 0
        self.peer_suite_passed = 0
    ...
    def setup_ui(self):
        ...
        self.btn_peer_suite = ttk.Button(peer, text="Run Self-Test Suite", command=self.start_peer_self_test)
        self.btn_peer_suite.grid(row=0, column=8, padx=5, pady=5)
        ttk.Label(
            peer,
            text="Fault injection: KoD, timestamps, mode/version, LI/stratum, drop/delay, offset, processing delay and replay",
        ).grid(row=1, column=0, columnspan=9, sticky="w", padx=5, pady=(2, 0))
    ...
    def start_peer_self_test(self):
        if self.peer_suite_running or (self.peer_thread and self.peer_thread.is_alive()):
            self.log_message("PEER SUITE: responder is already active.")
            return
        try:
            requested_port = int(self.peer_port_var.get())
        except (tk.TclError, ValueError):
            requested_port = 0
        self.peer_suite_running = True
        self.btn_peer_suite.config(state="disabled")
        self.btn_peer_start.config(state="disabled")
        threading.Thread(
            target=self.run_peer_self_test, args=(requested_port,), daemon=True
        ).start()
    ...
    def _suite_request():
        packet = bytearray(48)
        packet[0] = 0x23
        tx = ntp_timestamp(time.time())
        packet[40:48] = tx
        return bytes(packet), tx

    def _suite_exchange(self, port, timeout=1.5):
        request, tx = self._suite_request()
    ...
    def _suite_wait_started(self, port, timeout=2.0):
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            if self.peer_socket is not None:
                try:
                    return self.peer_socket.getsockname()[1]
                except OSError:
                    pass
            time.sleep(0.02)
        raise RuntimeError(f"responder failed to start on UDP/{port}")

    def _suite_stop_and_join(self):
        ...
        if self.peer_thread is not None:
            self.peer_thread.join(timeout=2.0)
        self.peer_thread = None
        time.sleep(0.03)

    def run_peer_self_test(self, requested_port):
        # This is a deterministic loopback regression suite for the responder itself.
        # It verifies the wire behavior generated by each fault mode. It does not claim
        # that an external NTP peer accepted/rejected a condition; peer-side logs remain
        # the authority for interoperability behavior.
        tests = [
            ("valid", lambda d, tx: len(d) == 48 and (d[0] & 7) == 4 and d[1] == 1 and d[24:32] == tx),
            ("kod", lambda d, tx: len(d) == 48 and d[1] == 0 and d[12:16] == b"RATE"),
            ("kod-deny", lambda d, tx: len(d) == 48 and d[1] == 0 and d[12:16] == b"DENY"),
            ("kod-rstr", lambda d, tx: len(d) == 48 and d[1] == 0 and d[12:16] == b"RSTR"),
            ("bad-originate", lambda d, tx: len(d) == 48 and d[24:32] != tx),
            ("short", lambda d, tx: len(d) == 16),
            ("wrong-mode", lambda d, tx: len(d) == 48 and (d[0] & 7) == 3),
            ("bad-version", lambda d, tx: len(d) == 48 and ((d[0] >> 3) & 7) == 0),
            ("zero-t2", lambda d, tx: len(d) == 48 and d[32:40] == b"\\x00" * 8),
            ("zero-t3", lambda d, tx: len(d) == 48 and d[40:48] == b"\\x00" * 8),
            ("li-alarm", lambda d, tx: len(d) == 48 and ((d[0] >> 6) & 3) == 3),
            ("stratum-16", lambda d, tx: len(d) == 48 and d[1] == 16),
        ]
        passed = failed = 0
        self.peer_suite_total = len(tests) + 7
        self.log_message(f"=== PEER RESPONDER SELF-TEST: {self.peer_suite_total} checks ===")

        def record(name, ok, detail=""):
            nonlocal passed, failed
            if ok:
                passed += 1
                self.log_message(f"[PASS] {name}" + (f" | {detail}" if detail else ""))
            else:
                failed += 1
                self.log_message(f"[FAIL] {name}" + (f" | {detail}" if detail else ""))

        try:
            for mode, validator in tests:
                self.peer_stop_event.clear()
                self.peer_thread = threading.Thread(
                    target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True
                )
                self.peer_thread.start()
                port = self._suite_wait_started(0)
                try:
                    data, tx = self._suite_exchange(port)
                    record(mode, bool(validator(data, tx)), f"response={len(data)} bytes")
                except Exception as exc:
                    record(mode, False, str(exc))
                finally:
                    self._suite_stop_and_join()

            # Duplicate T3: two different requests must receive the same server T3.
            mode = "duplicate"
            self.peer_stop_event.clear()
            self.peer_thread = threading.Thread(target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True)
            self.peer_thread.start(); port = self._suite_wait_started(0)
            try:
                d1, _ = self._suite_exchange(port); time.sleep(0.02); d2, _ = self._suite_exchange(port)
                record(mode, len(d1) == 48 and len(d2) == 48 and d1[40:48] == d2[40:48])
            except Exception as exc: record(mode, False, str(exc))
            finally: self._suite_stop_and_join()

            # Replay: second response must be byte-for-byte identical to the first.
            mode = "replay"
            self.peer_stop_event.clear()
            self.peer_thread = threading.Thread(target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True)
            self.peer_thread.start(); port = self._suite_wait_started(0)
            try:
                d1, _ = self._suite_exchange(port); time.sleep(0.02); d2, _ = self._suite_exchange(port)
                record(mode, d1 == d2 and len(d1) == 48)
            except Exception as exc: record(mode, False, str(exc))
            finally: self._suite_stop_and_join()

            # Drop: receiving no response is the expected result.
            mode = "drop"
            self.peer_stop_event.clear()
            self.peer_thread = threading.Thread(target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True)
            self.peer_thread.start(); port = self._suite_wait_started(0)
            try:
                try:
                    self._suite_exchange(port, timeout=0.35)
                    record(mode, False, "unexpected response")
                except socket.timeout:
                    record(mode, True, "request intentionally unanswered")
            finally: self._suite_stop_and_join()

            # Delayed response: require approximately the configured one-second delay.
            mode = "delayed"
            self.peer_stop_event.clear()
            self.peer_thread = threading.Thread(target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True)
            self.peer_thread.start(); port = self._suite_wait_started(0)
            try:
                start = time.monotonic(); data, _ = self._suite_exchange(port, timeout=2.0); elapsed = time.monotonic() - start
                record(mode, len(data) == 48 and elapsed >= 0.90, f"{elapsed:.3f}s")
            except Exception as exc: record(mode, False, str(exc))
            finally: self._suite_stop_and_join()

            # Timestamp offsets are checked against local wall time with a generous scheduler tolerance.
            for mode, expected in (("offset-plus-100ms", 0.100), ("offset-minus-100ms", -0.100)):
                self.peer_stop_event.clear()
                self.peer_thread = threading.Thread(target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True)
                self.peer_thread.start(); port = self._suite_wait_started(0)
                try:
                    before = time.time(); data, _ = self._suite_exchange(port); after = time.time()
                    t2 = ntp_timestamp_to_unix(data[32:40])
                    midpoint = (before + after) / 2.0
                    observed = t2 - midpoint
                    record(mode, abs(observed - expected) < 0.075, f"observed shift={observed*1000:+.1f} ms")
                except Exception as exc: record(mode, False, str(exc))
                finally: self._suite_stop_and_join()

            mode = "processing-100ms"
            self.peer_stop_event.clear()
            self.peer_thread = threading.Thread(target=self.run_peer_responder, args=("127.0.0.1", 0, mode), daemon=True)
            self.peer_thread.start(); port = self._suite_wait_started(0)
            try:
                data, _ = self._suite_exchange(port)
                t2 = ntp_timestamp_to_unix(data[32:40]); t3 = ntp_timestamp_to_unix(data[40:48])
                record(mode, 0.085 <= (t3 - t2) <= 0.250, f"T3-T2={(t3-t2)*1000:.1f} ms")
            except Exception as exc: record(mode, False, str(exc))
            finally: self._suite_stop_and_join()
        finally:
            self.peer_suite_passed, self.peer_suite_failed = passed, failed
            self.log_message(f"=== PEER RESPONDER SELF-TEST COMPLETE: {passed} passed / {failed} failed ===")
            self.peer_suite_running = False
            self.ui_queue.put(("peer_suite_done", None))
